...
1. **Chrome 已开启 CDP 远程调试**：`jupyterm setup` 通过 `http://127.0.0.1:{port}/json` 直接读取已有 Chrome 实例，不借助任何 MCP。
   - **Chrome 144+**：在地址栏打开 `chrome://inspect/#remote-debugging`，按提示允许远程调试（官方说明见 [Chrome 博客](https://developer.chrome.com/blog/chrome-devtools-mcp-debug-your-browser-session?hl=zh-cn)；与 `#devices` 不是同一入口）。
