...
DEFAULT_CONFIG = {
    ...
    # DevToolsActivePort 文件扫描失败时，HTTP 探测的备选端口列表
    "cdp_fallback_ports": [9222],
...
def _build_http_url(host: str, port: int, path: str) -> str:
    """构造 HTTP URL，统一处理 IPv4/IPv6 host。"""
    return f"http://{_normalize_host_for_url(host)}:{port}{path}"


def _build_ws_url(host: str, port: int, path: str) -> str:
    """构造 WebSocket URL，统一处理 IPv4/IPv6 host。"""
    return f"ws://{_normalize_host_for_url(host)}:{port}{path}"


def _get_json_from_hosts(port: int, path: str, timeout: float = 2.0):
    """按候选 host 轮询读取 JSON，成功返回 (json_data, host)。"""
    last_exc = None
    for host in CDP_HOST_CANDIDATES:
        try:
            ...
            last_exc = exc
    raise RuntimeError(last_exc or "no available CDP host")


def _resolve_ws_via_json_version(host: str, port: int) -> str | None:
    """通过同 host/port 的 /json/version 获取最新 browser websocket 地址。"""
    try:
        req = urllib.request.Request(
            _build_http_url(host, port, "/json/version"),
            headers={"Host": "localhost"},
        )
        ...
        if isinstance(ws_url, str) and ws_url.startswith("ws://"):
            ...
            # 部分环境返回 ws://localhost/devtools/...（无端口），这里补齐当前 CDP 端口。
            if parsed.port is None and parsed.hostname:
                return _build_ws_url(parsed.hostname, port, parsed.path or "/")
            return ws_url
    except Exception:
        return None
    return None
...
def _find_active_ports() -> list:
    """扫描 Chrome/Chromium 的 DevToolsActivePort 文件，找不到时 HTTP 探测备选端口。

    优先通过文件发现（chrome://inspect 勾选场景），无结果时 fallback 到 HTTP
    探测 cdp_fallback_ports（命令行 --remote-debugging-port 场景）。
    ...
    # Fallback: HTTP 探测 cdp_fallback_ports（适用于命令行显式端口启动的场景）
    config = load_config()
    fallback_ports = config.get("cdp_fallback_ports", [9222])
    for port in fallback_ports:
        try:
            data, _ = _get_json_from_hosts(port, "/json/version", timeout=2.0)
            ws_url = data.get("webSocketDebuggerUrl", "")
            if ws_url:
                parsed = urllib.parse.urlparse(ws_url)
                ws_path = parsed.path
...
class JscmdDaemon:
    ...
    def __init__(self):
        ...
        # KoKo 输入方式：True=SendTerminalData() JS API，False=Input.insertText
        self._use_send_terminal_data: bool = False
    ...
    async def connect(self, port: int, ws_path: str) -> bool:
        ...
        # 优先使用 DevToolsActivePort 给出的 ws_path，按 IPv4/IPv6 候选主机重试。
        for host in CDP_HOST_CANDIDATES:
            url = _build_ws_url(host, port, ws_path)
            ...
            try:
                ...
                self._cdp_host = host
                print(f"[daemon] Chrome 连接成功 endpoint={url}", file=sys.stderr)
                return True
            except Exception as exc:
                last_exc = exc
                # ws_path 过期常见表现是 404，此时用 /json/version 获取最新 ws 地址重试一次。
                if "HTTP 404" in str(exc):
                    recovered = _resolve_ws_via_json_version(host, port)
                    if recovered and recovered != url:
                        tried_urls.append(recovered)
                        print(f"[daemon] 404 自动修复 endpoint: {recovered}", file=sys.stderr)
                        try:
                            ws = await ws_connect(
                                recovered, open_timeout=15,
                                ping_interval=None, ping_timeout=None,
                            )
                            parsed = urllib.parse.urlparse(recovered)
                            self.browser_ws = ws
                            self._cdp_port = port
                            self._cdp_ws_path = parsed.path or ws_path
                            self._cdp_host = parsed.hostname or host
                            print(f"[daemon] Chrome 连接成功 endpoint={recovered}", file=sys.stderr)
                            return True
                        except Exception as recover_exc:
                            last_exc = recover_exc

        raise RuntimeError(
            f"Chrome CDP 连接失败，已尝试 endpoints={tried_urls}, last_error={last_exc}"
        )
    ...
    async def find_luna_page(self) -> bool:
        """仅基于当前活跃页面检测 JumpServer 并附加 session。

        规则：
        1. 只检查 visibilityState=visible 的当前页面；
        2. 不扫描其他页面，不按 URL 子串猜测；
        3. 当前页不是 JumpServer 就直接失败返回。
        """
        use_disp = self._disp is not None and self._disp._running

        async def _send_req(session_id: str, method: str, params: dict) -> dict:
            msg = {"id": self._nid(), "method": method, "params": params}
            if session_id:
                msg["sessionId"] = session_id
            if use_disp:
                resp = await self._disp.request(msg, timeout=10.0)
                return resp.get("result", {}) if isinstance(resp, dict) else {}
            await self.browser_ws.send(json.dumps(msg))
            resp = await self._recv_until(msg["id"])
            return resp.get("result", {}) if isinstance(resp, dict) else {}

        async def _attach_target(target_id: str) -> str:
            res = await _send_req("", "Target.attachToTarget", {"targetId": target_id, "flatten": True})
            return res.get("sessionId", "")

        async def _detach_session(sid: str) -> None:
            if not sid:
                return
            try:
                await _send_req("", "Target.detachFromTarget", {"sessionId": sid})
            except Exception:
                pass

        async def _eval_on_target(target_id: str, expression: str):
            sid = await _attach_target(target_id)
            if not sid:
                return None
            try:
                await _send_req(sid, "Runtime.enable", {})
                res = await _send_req(sid, "Runtime.evaluate", {"expression": expression, "returnByValue": True})
                obj = res.get("result", {})
                if obj.get("type") in ("string", "boolean", "number"):
                    return obj.get("value")
                return None
            finally:
                await _detach_session(sid)
        ...
        # 1. 取 page targets
        targets_res = await _send_req("", "Target.getTargets", {})
        targets = targets_res.get("targetInfos", [])
        page_targets = [t for t in targets if t.get("type") == "page"]
        if not page_targets:
            print("[daemon] 未找到任何 page target。", file=sys.stderr)
            return False
        ...
        for t in page_targets:
            ...
            if not tid:
                continue
            state = await _eval_on_target(tid, "document.visibilityState")
            if state == "visible":
                selected = t
                break

        if not selected:
            print("[daemon] 未找到当前活跃页面（visible）。", file=sys.stderr)
            return False

        selected_tid = selected.get("targetId", "")
        selected_url = selected.get("url", "")

        # 3. 当前页 JumpServer 特征检测（不依赖 /luna/）
        probe_js = (
            "(() => {"
            "  const href = (location.href || '').toLowerCase();"
            "  const ifr = Array.from(document.querySelectorAll('iframe'));"
            "  const hasKokoIframe = ifr.some(f => {"
            "    const s = (f.getAttribute('src') || f.src || '').toLowerCase();"
            "    return s.includes('/koko/') || s.includes('koko');"
            "  });"
            "  const hasLunaPath = href.includes('/luna/');"
            "  return JSON.stringify({hasKokoIframe, hasLunaPath});"
            "})()"
        )
        probe_text = await _eval_on_target(selected_tid, probe_js)
        is_jump = False
        if isinstance(probe_text, str):
            try:
                info = json.loads(probe_text)
                # 收敛判定，避免把 Jupyter/其他终端页误识别成 JumpServer。
                is_jump = bool(info.get("hasKokoIframe") or info.get("hasLunaPath"))
            except Exception:
                is_jump = False

        if not is_jump:
            print(f"[daemon] 当前活跃页不是 JumpServer: {selected_url[:120]}", file=sys.stderr)
            return False

        # 4. 附加当前活跃页作为长期 session
        self.luna_target_id = selected_tid
        self.session_id = await _attach_target(self.luna_target_id)
        if not self.session_id:
            print("[daemon] 附加当前活跃页 session 失败。", file=sys.stderr)
            return False

        print(f"[daemon] 当前活跃页已附加: {selected_url[:80]}", file=sys.stderr)
    ...
    async def _order_contexts(self, contexts_raw: list) -> list:
        ...
        # 检测 KoKo 输入方式：是否有 SendTerminalData JS API
        if ordered:
            probe = await self._eval(
                "typeof SendTerminalData === 'function' ? 'yes' : 'no'",
                ordered[0][0], timeout=3.0)
            self._use_send_terminal_data = (probe == "yes")
            if self._use_send_terminal_data:
                print("[daemon] KoKo 输入方式: SendTerminalData() JS API",
                      file=sys.stderr)
            else:
                print("[daemon] KoKo 输入方式: Input.insertText (CDP)",
                      file=sys.stderr)
...
"""
    ...
    async def _send_line(self, text: str, ctx_id: int = None):
        """向终端发送一行文本并按 Enter（fire-and-forget，支持并发）。

        两种输入方式自动适配：
        - SendTerminalData() JS API（新版 KoKo /koko/terminal/）
        - Input.insertText + Enter（旧版 KoKo /koko/connect/）

        @param[in] text   要发送的文本（不含换行符）
        @param[in] ctx_id iframe execution context id（SendTerminalData 模式必需）
        @return none
        """
        if self._use_send_terminal_data and ctx_id is not None:
            js_text = json.dumps(text + "\r")
            await self._eval(
                f"SendTerminalData({js_text})",
                ctx_id, timeout=3.0, fire_and_forget=True)
    ...
    async def _send_special_key(self, ctx_id: int, key_name: str):
        ...
        两种输入方式自动适配：
        - SendTerminalData(ctrl_char)（新版 KoKo）
        - Input.dispatchKeyEvent + modifiers（旧版 KoKo）
        ...
        if self._use_send_terminal_data:
            char_code = ord(ctrl_char)
            await self._eval(
                f"SendTerminalData(String.fromCharCode({char_code}))",
                ctx_id, timeout=3.0, fire_and_forget=True)
    ...
    async def get_tab_count(self) -> int:
        ...
        @return iframe 数量
        """
        js = "document.querySelectorAll('iframe[src*=\"/koko/\"]').length"
        result = await self._eval(js, 0, timeout=5.0)
    ...
    async def exec_command(self, tab_idx: int, cmd: str, timeout: float = 30.0) -> str:
        ...
        try:
            ...
            try:
                ...
                for line in lines:
                    await self._send_line(line, ctx_id)
                ...
                if timed_out:
                    ...
                    await self._send_line(flush_cmd, ctx_id)
...
"""
    ...
    async def _ensure_luna(self) -> dict:
        """确保 Luna session 已加载，未加载时尝试懒加载。

        只检查 session_id（表示已附着 Luna CDP session）。iframe_contexts 为空
        说明没有打开终端标签，但 Luna 本身已就绪，后续命令会得到"无标签"的错误。

        @return None 如果成功，否则返回错误响应 dict
        """
        if self.session_id:
            return None
        if await self.find_luna_page():
            ...
            return None
        return {"ok": False,
                "error": "未检测到 JumpServer Luna 页面。"
                         "请在 Chrome 中打开 JumpServer（URL 含 /luna/）后重试。"}

    async def handle_request(self, req: dict) -> dict:
        """处理来自 CLI 的请求，返回响应。

        支持两类命令：
        - 通用 CDP 命令（cdp_pages / cdp_eval）：无需 Luna，任何有 Chrome CDP 的场景可用
        - JumpServer 命令（exec / list / connect 等）：需要 Luna，首次调用时懒加载
        ...
        if cmd == "ping":
            luna_ok = bool(self.session_id and self.iframe_contexts)
            live = (await self._live_tab_info()) if luna_ok else []
            return {"ok": True, "tabs": len(live), "luna": luna_ok}

        if cmd == "cdp_pages":
            try:
                host = self._cdp_host or "127.0.0.1"
                req_obj = urllib.request.Request(
                    _build_http_url(host, self._cdp_port, "/json"),
                    headers={"Host": "localhost"})
                ...
                return {"ok": False, "error": f"获取 page targets 失败: {e}"}
        ...
        if cmd == "cdp_eval":
            ...
            if not target_id or not expression:
                return {"ok": False,
                        "error": "cdp_eval 需要 target_id 和 expression"}
            try:
                async def _send_req(session_id: str, method: str, params: dict) -> dict:
                    """统一 CDP 发送入口：有 dispatcher 时走 dispatcher，避免并发 recv 冲突。"""
                    msg = {"id": self._nid(), "method": method, "params": params}
                    if session_id:
                        msg["sessionId"] = session_id
                    if self._disp:
                        resp = await self._disp.request(msg)
                        return resp.get("result", {}) if isinstance(resp, dict) else {}
                    return await _cdp_send(self.browser_ws, session_id, method, params, msg["id"])

                attach_resp = await _send_req(
                    "", "Target.attachToTarget",
                    {"targetId": target_id, "flatten": True},
                )
                sid = attach_resp.get("sessionId", "")
                if not sid:
                    return {"ok": False,
                            "error": f"attachToTarget 失败: {attach_resp}"}
                try:
                    # 启用 Runtime 获取默认 executionContext
                    await _send_req(sid, "Runtime.enable", {})
                    eval_resp = await _send_req(
                        sid, "Runtime.evaluate",
                        {"expression": expression, "returnByValue": True},
                    )
                    result_obj = eval_resp.get("result", {})
                ...
                return {"ok": False, "error": f"cdp_eval 失败: {e}"}

        # --- JumpServer 专用命令（需要 Luna，首次调用时懒加载） ---

        if cmd == "list":
            err = await self._ensure_luna()
            if err:
                return err
    ...
    def _read_active_port_file() -> str:
        """读取 DevToolsActivePort 文件内容用于心跳快照比对，返回空串表示不可用。

        优先读文件（chrome://inspect 场景），文件不存在时 fallback 到 HTTP 探测
        /json/version（命令行 --remote-debugging-port 场景）。Chrome 重启后
        webSocketDebuggerUrl 中的 UUID 会变，快照对比仍能触发重连。
        ...
        for pattern in (
            ...
            if os.path.exists(p):
                try:
                    ...
                    pass
        # Fallback: HTTP 探测作为等效快照
        config = load_config()
        for port in config.get("cdp_fallback_ports", [9222]):
            try:
                data, _ = _get_json_from_hosts(port, "/json/version", timeout=2.0)
...
async def _daemon_main(idle_timeout: float = -1.0):
    ...
    作为通用 CDP 网关启动：Chrome 连接是必须的，但 JumpServer Luna 页面是可选的。
    若启动时未检测到 Luna，daemon 以通用 CDP 网关模式运行（支持 cdp_pages / cdp_eval），
    JumpServer 专用命令在首次调用时再尝试懒加载 Luna。
    ...
    ports = _find_active_ports()
    if not ports:
        print("[daemon] 未找到 Chrome CDP 实例。\n"
              "  请选择以下任一方式开启远程调试：\n"
              "  方式 A：chrome://inspect/#remote-debugging 勾选允许\n"
              "  方式 B：命令行启动 Chrome 时加 --remote-debugging-port=9222",
              file=sys.stderr)
        sys.exit(1)
    ...
    # 连接 Chrome CDP WebSocket（必须成功）
    chrome_connected = False
    for port, ws_path in ports:
        for attempt in range(max_retries):
            try:
                await asyncio.wait_for(
                    daemon.connect(port, ws_path),
    ...
    if not chrome_connected:
        print("[daemon] 无法连接 Chrome CDP。\n"
              "  请确认：\n"
              "  1. Chrome 已开启远程调试\n"
              "  2. Chrome 安全弹窗已点击「允许」", file=sys.stderr)
        sys.exit(1)

    # Luna 检测：可选，找不到不退出（以通用 CDP 网关模式运行）
    luna_found = await daemon.find_luna_page()
    if luna_found:
        await daemon.setup_monitoring()
    else:
        print("[daemon] 未检测到 JumpServer Luna 页面，"
              "以通用 CDP 网关模式启动（cdp_pages / cdp_eval 可用）",
              file=sys.stderr)
