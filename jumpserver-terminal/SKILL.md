---
name: jumpserver-terminal
description: 通过 Chrome CDP 与 JumpServer Web 终端交互，执行命令并返回输出，同时提供通用 CDP 网关能力。
---
...
jscmd daemon 同时作为**通用 Chrome CDP 网关**，提供 `cdp_pages` / `cdp_eval` 命令供其他 Skill（如 jupyterlab-terminal）复用 CDP 连接。

---

## 前提条件

1. **Chrome 远程调试已开启**（任选其一）：

   **方式 A：chrome://inspect 勾选（推荐，Chrome 144+）**
   打开 `chrome://inspect/#remote-debugging`，勾选"Allow remote debugging for this browser instance"。
   验证：
   ```bash
   ...
   **方式 B：命令行启动（适合自动化场景）**
   ...
   > **两种方式的区别**：方式 A 在默认 profile 写入 `DevToolsActivePort` 文件，daemon 通过文件扫描发现端口；方式 B 不写文件（显式端口无需通告），daemon 通过 HTTP fallback 探测 `cdp_fallback_ports`（默认 `[9222]`，可在 `~/.jscmd_config.json` 中自定义）。两种可共存，互不冲突。

2. **使用 JumpServer 功能时**：浏览器已打开 JumpServer（URL 含 `/luna/`）。不打开 JumpServer 时，daemon 以通用 CDP 网关模式运行（`cdp_pages` / `cdp_eval` 可用）。

3. **依赖安装**（仅需 websockets）：
   ```bash
...
daemon 同时作为通用 Chrome CDP 网关，以下命令通过 `~/.jscmd.sock` Unix socket 调用：

| daemon 命令 | 请求格式 | 说明 |
|------------|----------|------|
| `cdp_pages` | `{"cmd": "cdp_pages"}` | 返回所有 page targets（`{"ok": true, "pages": [...]}`) |
| `cdp_eval` | `{"cmd": "cdp_eval", "target_id": "...", "expression": "..."}` | 在指定 page 上执行 JS 表达式（`{"ok": true, "value": ..., "type": "..."}`) |

这些命令不依赖 JumpServer Luna，只要 Chrome 已开启 CDP 即可使用。`jupyterlab-terminal` 的 `setup` 阶段即通过这两个命令自动发现 Jupyter 页面和提取 token。
